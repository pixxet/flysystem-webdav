...
class WebDAVAdapter extends AbstractAdapter
{
     ...
     */
    function __construct(Client $client, $prefix = null, $useStreamedCopy = true)
    {
        $this->client = $client;
    ...
    function read($path)
    {
        ...
        try {
            ...
            $timestamp_key = isset($response['headers']['last-modified']) ? 'last-modified' : 'date';

            return array_merge([
                'contents'  => $response['body'],
                'timestamp' => strtotime(is_array($response['headers'][$timestamp_key])
                    ? current($response['headers'][$timestamp_key])
                    : $response['headers'][$timestamp_key]),
                'path'      => $path,
     ...
     * {@inheritdoc}
     */
    function write($path, $contents, Config $config)
    {
     ...
     * {@inheritdoc}
     */
    function update($path, $contents, Config $config)
    {
     ...
     * {@inheritdoc}
     */
    function createDir($path, Config $config)
    {
     ...
     * {@inheritdoc}
     */
    function listContents($directory = '', $recursive = false)
    {
        ...
        foreach ($response as $path => $object) {
            ...
            // dump($this->removePathPrefix($path));
     ...
     */
    function setUseStreamedCopy($useStreamedCopy)
    {
     ...
     * @return bool
     */
    function nativeCopy($path, $newPath)
    {
     ...
     * @return array
     */
    function normalizeObject(array $object, $path)
    {
